...
export async function scheduledSync(): Promise<void> {
  ...
  try {
    ...
    if (connections.length === 0) {
      console.log('[Sync] No active Microsoft connections found');
      return;
    }
    ...
    for (const connection of connections) {
      try {
        ...
        console.log(`[Sync] [${connection.tenantId}] Fetching audit events from ${startTime.toISOString()} to ${endTime.toISOString()}`);

        const events = await managementApi.fetchAuditEvents(startTime, endTime);

        console.log(`[Sync] [${connection.tenantId}] Fetched ${events.length} events from O365`);
        ...
        for (const event of events) {
          try {
            ...
            console.error(`[Sync] [${connection.tenantId}] Failed to store event ${event.Id}:`, err);
          }
        }
        ...
        console.log(`[Sync] [${connection.tenantId}] Synced ${newCount} new events (${duplicateCount} duplicates)`);
      } catch (err) {
        console.error(`[Sync] [${connection.tenantId}] Failed to sync:`, err);
      }
    }

    console.log(`[Sync] Completed: ${totalNewEvents} new events across ${connections.length} tenants`);
  } catch (error) {
 ...
 */
export async function scheduledAnomalyDetection(): Promise<void> {
  console.log('[Anomaly] Starting scheduled anomaly detection...');

  try {
    // Check ML service health first
    const mlHealthy = await checkHealth();
    if (!mlHealthy) {
      console.warn('[Anomaly] ML service unavailable, skipping anomaly detection');
      return;
    }
    ...
    if (unprocessedEvents.length === 0) {
      console.log('[Anomaly] No unprocessed events to analyze');
      return;
    }

    console.log(`[Anomaly] Processing ${unprocessedEvents.length} audit events for anomaly detection`);
    ...
    console.log(`[Anomaly] Completed: ${anomaliesCreated} anomalies, ${alertsCreated} alerts`);
  } catch (error) {
    console.error('[Anomaly] Scheduled anomaly detection failed:', error);
    throw error;
  }
}
